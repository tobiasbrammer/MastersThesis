...
def preprocessMonthlyData(
    ...
    if normalizeCharacteristics:
        ...
        _df = _df.groupby('date').apply(lambda x: x.rank(method="first") / x.count() - 0.5) # DLSA does it differently
        df = _df.copy()
    else:
...
def preprocessDailyReturns(
    ...
    tmask = np.load(os.path.join(logdir, "factor_data/MonthlyDataTickers.npy"), allow_pickle=True)
...
class IPCA:
    def __init__(
        ...
        self.monthlyCaps = self.monthlyData[:, :, 6]
    ...
    def compute_weight_matrices(self, mask):
        ...
        for month in range(len(self.monthlyDates)):
            ...
            rmonth = self.dailyData[:, mask[month, :]][
                     idxs_days_month, :
                     ]  # TtxNt matrix
    ...
    def DailyOOSRollingWindow(
        ...
        firstOOSDailyIdx = np.argmax(
            self.dailyDates
            >= (
                    # ToDo: No attribute. Fix this.
                    pd.to_datetime(datetime.datetime(self.dailyDates.year[0], self.dailyDates.month[0], 1))
        ...
        Rdaily = self.dailyData[,firstOOSDailyIdx:, :] # ToDo: KeyError: '(slice(0, None, None), slice(None, None, None)) is not a file in the archive'
        ...
        DataTrain = np.load(
            os.path.join(
                "residuals",
                "ipca_normalized",
                f"IPCA_DailyOOSresiduals_1_factors_{initialMonths}_initialMonths_{sizeWindow}_window_{reestimationFreq}_reestimationFreq_{CapProportion}_cap.npy",
            )
        )
        assetsToConsider = (
                ...
                for nWindow in range(nWindows):
                    ...
                    if not skip_oos:
                        ...
                        for month in range(
                            ...
                            MatrixFull[
                                np.broadcast_to(
                                    mask[month - 1, :].reshape((N, 1))
                                    @ mask[month - 1, :].reshape((1, N)),
                                    (Tprime, N, N),
                                    )
                            ] = (
                                    mask_month
