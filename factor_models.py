...
def run_factor_models():
    ...
    # Run PCA
    # run_pca(listFactors=factor_list, sizeWindow=sizeWindow, capProportion=capProportion)
    ...
    # Run IPCA
    # run_ipca(listFactors=factor_list, sizeWindow=sizeWindow, capProportion=capProportion)
...
def run_pca(listFactors: list, sizeWindow: list, capProportion: list):
...
class PCA:
    ...
    def OOSRollingWindowPermnos(
        ...
        for factor in listFactors:
            ...
            if not os.path.exists("/factor_data/resi"):
                os.makedirs("/factor_data/outputs")

            np.save(
                os.path.join(
                    os.getcwd() + "/factor_data/outputs",
...
class IPCA:
    ...
    def DailyOOSRollingWindow(
        ...
        tol=1e-02,
        initialMonths=35 * 12,
        sizeWindow=15 * 12,  # Original value was 15*12
        CapProportion=0.001,
        save_beta=False,
        save_gamma=False,
        save_rmonth=False,
...
class FamaFrench:
    ...
    def OOSRollingWindowPermnos(
        ...
        # filename = f"DailyFamaFrench_OOSresiduals_{listFactors}_factors_{initialOOSYear}_initialOOSYear_{sizeWindow}_rollingWindow_{cap}_Cap.npy"
        DataTrain = Rdaily  # ToDo: What else to load?
