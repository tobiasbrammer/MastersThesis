...
def get_wrds(start_date="1997-12-31", end_date="2024-01-01"):
    import pandas as pd
    import numpy as np
    import wrds as wrds
    ...
    crsp_m['cor1'] = \
        crsp_m.groupby('permno')['logret'].rolling(750, min_periods=750).corr(crsp_m['laglogret']).reset_index()[
            'logret']
    crsp_m['cor2'] = \
        crsp_m.groupby('permno')['logret'].rolling(750, min_periods=750).corr(crsp_m['laglogret2']).reset_index()[
            'logret']
    crsp_m['cor3'] = \
        crsp_m.groupby('permno')['logret'].rolling(750, min_periods=750).corr(crsp_m['laglogret3']).reset_index()[
            'logret']

    crsp_m['cor'] = (crsp_m['cor1'] + crsp_m['cor2'] + crsp_m['cor3']) / 3

    # Calculate Beta
    crsp_m['beta'] = crsp_m['cor'] * (crsp_m['vol'] / crsp_m['vol'].shift(1))
    ...
    # Save to parquet
    comp.to_parquet('factor_data/fundamentals.parquet')
