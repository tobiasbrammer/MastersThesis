...
def estimate(
    ...
    # "Logging"
    print(
        f"Estimating: {estimate_start_idx}:{min(estimate_start_idx + length_training, T)} for {model_tag}"
    )
    print(
        f"Testing: {estimate_start_idx + length_training - lookback}:"
    ...
    residual_weights_dev = (
        ...
        if residual_weights is None
            ...
            (estimate_start_idx + length_training - lookback) : min(
                estimate_start_idx + length_training + test_size, T
            )
        ]
    )
    ...
    # Train the model
    model1 = model
    returns, turnovers, short_proportions, all_weights, a2t = train(
        model1,
    ...
    print(
        f"Sharpe: {full_sharpe * np.sqrt(252):0.2f}, ret: {full_ret * 252:0.4f}, std: {full_std * np.sqrt(252):0.4f}"
        ...
        for i in range(int((T - lookback) / batchsize) + 1):
            ...
            if residual_weights_train is None:
                ...
                assert (
                    weights.shape
                    == residual_weights_train[
                        (lookback + batchsize * i) : (
                            min(lookback + batchsize * (i + 1), T)
                        ),
                        :,
                        0,
                    ].shape
                ...
                weights2 = torch.bmm(
                    ...
                    torch.tensor(
                        residual_weights_train[
                            (lookback + batchsize * i) : min(
                                lookback + batchsize * (i + 1), T
                            )
                        ]
                    ),
                ).squeeze()  # will be T1xN2
            ...
            if objective == "sharpe":
                ...
                loss = (
                    -mean_ret * 252 + std * 15.9
                )  # Hvor kommer 15.9 fra? 252 er antal handelsdage på et år
            ...
            turnover[(batchsize * i) : (min(batchsize * (i + 1), T - lookback)) - 1] = (
            ...
            if log_dev_progress and epoch % log_dev_progress_freq == 0:
                ...
                if early_stopping:
                    ...
                    if (
                    ...
                    ):  # It will always start here, as we have min_dev_loss = np.inf
                        patience = 0
                        ...
                        if trial == early_stopping_max_trials:
                            ...
                            trial += 1
                            print(
                                f"Trial {trial} of {early_stopping_max_trials} - Reducing learning rate"
                            )
...
def get_returns(
    ...
    with torch.no_grad():
        ...
        ):  # This ensembles if many checkpoints are given (whatever that means)
            if load_params:
        ...
        # noinspection PyArgumentList
        rets_test = torch.sum(
            weights * torch.tensor(df_test[lookback:T, :], device=device), axis=1
        )
...
def test(
    ...
    for t in range(int((T - length_training) / retrain_freq) + 1):
        ...
        data_test_t = Data[
            (length_training + t * retrain_freq - lookback) : min(
                length_training + (t + 1) * retrain_freq, T
            )
        ]
        if residual_weights is not None:
            residual_weights_train_t = residual_weights[
                (t * retrain_freq) : (length_training + t * retrain_freq)
            ]
            residual_weights_test_t = residual_weights[
                (length_training + t * retrain_freq - lookback) : min(
                    length_training + (t + 1) * retrain_freq, T
                )
            ]
...
def run_model(
    ...
    for i in range(len(factors)):
        ...
        results_dict[model_tag] = {
            "returns_train": rets_train,
            "returns_test": rets_test,
            ...
            "std_train": std_train,
            "std_test": std_test,
            "turnover_train": turnover_train,
            "turnover_test": turnover_test,
            "short_proportion_train": short_proportion_train,
            "short_proportion_test": short_proportion_test,
        }
