...
def preprocess_ou(df, lookback):
    ...
    for t in range(lookback, T):
        ...
        Nx, Tx = x.shape # Nx is assigned the number of rows in x, and Tx is assigned the number of columns.
        Ys = x[:, 1:] # Separate target variables in a dataset from the feature variables.
        Xs = x[:, :-1] # Separate feature variables in a dataset from the target variables.
        ...
        mus = cs / (1 - bs + 0.000001) # ToDo: What is this transformation?
        mask = (bs > 0) * (bs < 1)  # Create a mask to select the features that have a coefficient between 0 and 1.
        # kappas = -np.log(bs)/Deltat # if bs between 0 and 1
        residuals = Ys - bs.reshape((Nx, 1)) * Xs - cs.reshape((Nx, 1))  # (N,T-1)
        sigmas = np.sqrt(
            np.var(residuals, axis=1) / np.abs(1 - bs**2 + 0.000001)
        )
        ...
        windows[t - lookback, idxs, 0] = Ys[:, -1] # Update the windows array with the target variables.
        windows[t - lookback, idxs, 1] = mus # Update the windows array with the transformed residuals.
        windows[t - lookback, idxs, 2] = sigmas # Update the windows array with the standard deviation of the residuals.
        windows[t - lookback, idxs, 3] = R2 # Update the windows array with the R-squared value.
        #
