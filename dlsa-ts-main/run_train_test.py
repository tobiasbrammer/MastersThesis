...
def configure_logging(
    app_name: str, run_id: str = None, logdir: str = "logs", debug=False
):
...
def run(
    ...
    model_name = config[
        "model_name"
    ]  # name of a class defined in models folder and initialized in model folder's __init__.py
    results_tag = config[
        "results_tag"
    ]  # optional; try not to use underscores in this tag, use dashes instead
    debug = config["debug"]  # set to True to turn on debug logging and file naming

    # start logging
    username, hostname, log_tag, starttime = (
        configure_logging(model_name, run_id=run_id, debug=debug)
        if run_id
        else initialize_logging(model_name, debug=debug)
    )
    ...
    factor_models = config[
        "factor_models"
    ]  # number of factors per residual time series to test, for each factor model
    cap = config[
        "cap_proportion"
    ]  # defines asset universe: 0.01 corresponds to a residual data set
    use_residual_weights = config[
        "use_residual_weights"
    ]  # use residual composition matrix to compute turnover, short proportion, etc.
    objective = config[
        "objective"
    ]  # objective function: 'sharpe' or 'meanvar' or 'sqrtMeanSharpe'
    ...
    for factor in factor_models["IPCA"]:
        ...
        w = 20 * 12  # window size
        filepaths += [
            f"{cwd}/residuals/{ipcadir}/{ipcartag}_{factor}_factors_{im}_initialMonths_{w}_window_12_reestimationFreq_{cap}_cap.npy"
        ]
        datanames += ["IPCA" + str(factor)]
        residual_weightsNames += [
            f"{cwd}/residuals/{ipcadir}/{ipcamtag}_{factor}_factors_{im}_initialMonths_{w}_window_12_reestimationFreq_{cap}_cap.npy"
        ]
    # PCA
    pcadir = "pca"
    ...
    for factor in factor_models["PCA"]:
        ...
        cw = 252
        filepaths += [
            f"{cwd}/residuals/{pcadir}/{pcartag}_{factor}_factors_{ioy}_initialOOSYear_{w}_rollingWindow_{cw}_covWindow_{cap}_Cap.npy"
        ]
        datanames += ["PCA" + str(factor)]
        residual_weightsNames += [
            f"{cwd}/residuals/{pcadir}/{pcamtag}_{factor}_factors_{ioy}_initialOOSYear_{w}_rollingWindow_{cw}_covWindow_{cap}_Cap.npy"
        ]
    # FamaFrench
    ffdir = "famafrench"
    ...
    for factor in factor_models["FamaFrench"]:
        ...
        w = 60
        filepaths += [
            f"{cwd}/residuals/{ffdir}/{ffrtag}_{factor}_factors_{ioy}_initialOOSYear_{w}_rollingWindow_{cap}_Cap.npy"
        ]
        ...
        # datanames += ['FamaFrenchNew'+str(factor)]  # see line 578 in train_test.py
        ...
        residual_weightsNames += [
            f"{cwd}/residuals/{ffdir}/{ffmtag}_{factor}_factors_{ioy}_initialOOSYear_{w}_rollingWindow_{cap}_Cap.npy"
        ]
    ...
    daily_dates = pd.to_datetime(
        FamaFrenchDailyData.index[
            (FamaFrenchDailyData.index > 19980000)
            & (FamaFrenchDailyData.index < 20170000)
        ],
        format="%Y%m%d",
    )  # 4,781 days in Ken French Factor file btwn 1/1/1998 & 12/31/2016
    ...
    for i in range(len(filepaths)):
        ...
        if "perturbation" in config and len(config["perturbation"]) > 0:
            logging.info(
                f"Before perturbing residuals: std: {np.std(residuals[residuals != 0]):0.4f}"
            )
            residuals = perturb(residuals, config["perturbation"])
            logging.info(
                f"After perturbing residuals: std: {np.std(residuals[residuals != 0]):0.4f}"
            )
        ...
        if gpu_device_ids is None:
            if config["model"]["lookback"] == 30 and config["length_training"] == 1000:
                ...
                config["model"]["lookback"] == 30 and config["length_training"] >= 2000
            ):
                num_gpus_needed = 3
            elif (
                config["model"]["lookback"] == 60 and config["length_training"] == 1000
            ):
                num_gpus_needed = 4
            else:
                logging.error(
                    "Unknown context for estimating number of GPUs needed for training"
                )
                num_gpus_needed = int(
                    input("Enter number of GPUs needed for model (integer):")
                )
                logging.info(
                    f"User entered '{num_gpus_needed}' GPUs needed for this model's training"
                )
            device_ids = get_free_gpu_ids(min_memory_mb=1500)[:num_gpus_needed]
        else:
            device_ids = gpu_device_ids

        # prepare output folder
        outdir = os.path.join(
            str(pathlib.Path().resolve()), "results", config["model_name"]
        )
        ...
        if config["mode"] == "test":  # can be 'test' or 'estimate'
        ...
        ):  # used to train a model once, e.g. for hyperparameter exploration on dev dataset
            ...
            raise Exception(
                f"Invalid mode '{config['mode']}'; must be either 'test' or 'estimate'"
            )
...
def init_argparse():
    ...
    parser.add_argument(
        "--config",
        "-c",
        help="path to a .yaml configuration file (e.g. 'config/cnntransformer-full.yaml')",
        required=True,
    )
    parser.add_argument(
        "--run-id",
        "-r",
        help="identifier string carrying external information (e.g. 'run42')",
        required=False,
    )
    parser.add_argument(
        "--gpu-device-ids",
        ...
        nargs="*",
        type=int,
        help="space-separated list of GPU device IDs to use (e.g. '0 1 2 3')",
        required=False,
    )
    parser.add_argument(
        "--notification-phone-number",
        "-p",
        help="notification phone number string (e.g. '+12345678900')",
        required=False,
    )
    return parser
