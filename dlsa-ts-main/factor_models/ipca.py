...
def preprocessDailyReturns(
    ...
    _UNK=np.nan,
    logdir=os.getcwd(),
    name="DailyReturns-RFadjusted.npz",
):
    savepath = os.path.join(logdir, name)
    if os.path.exists(savepath):
    ...
    def MonthlyOOSExpandingWindow(
        ...
        for nFactors in listFactors:
            ...
            for nWindow in range(nWindows):
                if nWindow == 0:
                    ...
                    while nIter < maxIter:
                        ...
                        f_list, _ = self._step_factor(
                            R_list[:initialMonths], I_list[:initialMonths], Gamma
                        )
                        nIter += 1
                ...
                f_list, residual_list = self._step_factor(
                    R_list[
                ...
                residualsOOS[(nWindow * sizeWindow) : ((nWindow + 1) * sizeWindow), :][
                    mask[
                        (initialMonths + nWindow * sizeWindow) : (
                            initialMonths + (nWindow + 1) * sizeWindow
                        ),
                ...
                ] = np.squeeze(np.concatenate(residual_list))
            ...
            if save:
                np.save(
                    os.path.join(
                        self._logdir,
                        "IPCA_OOSresiduals_%d_factors_%d_initialMonths_%d_window.npy"
                        % (nFactors, initialMonths, sizeWindow),
                    ),
                    residualsOOS,
                )
        return residualsOOS
