        ...
        for factor in factorList:
            ...
            for t in range(T - firstOOSDailyIdx):
                ...
                if factor == 0:
                    ...
                    factors = np.dot(
                        res_cov_window[-sizeWindow:, :] / res_vol, loadings
                    )
                    DayFactors = np.dot(
                ...
                if t % 50 == 0 and printOnConsole:
                    print(
                        f"At date {t}/{T-firstOOSDailyIdx}, Number of permnos: {np.sum(idxsSelected)}"
                    )
                    print(
                        np.linalg.norm(
                            residualsOOS[t : (t + 1), assetsToConsider]
                            - residualsMatricesOOS[t : (t + 1)]
            ...
            if save:
                if factor == 20:
                    np.save(
                        os.path.join(
                            self._logdir,
                            f"AvPCA_OOSresiduals_{factor}_factors_{initialOOSYear}_initialOOSYear_{sizeWindow}_rollingWindow_{sizeCovarianceWindow}_covWindow_{CapProportion}_Cap.npy",
                        ),
                        residualsOOS,
                    )
                np.save(
                    os.path.join(
                        self._logdir,
                        f"AvPCA_OOSMatrixresiduals_{factor}_factors_{initialOOSYear}_initialOOSYear_{sizeWindow}_rollingWindow_{sizeCovarianceWindow}_covWindow_{CapProportion}_Cap.npy",
                    ),
                    residualsMatricesOOS,
                )
        return

    def OOSRollingWindowPermnosVectorized(
        self,
        save=True,
        printOnConsole=True,
        initialOOSYear=1998,
        ...
        for t in range(T - firstOOSDailyIdx):
            ...
            if t % 50 == 0 and printOnConsole:
                print(
                    f"At date {t}/{T-firstOOSDailyIdx}, Not-missing permnos: {np.sum(idxsNotMissingValues)}, Permnos with cap {np.sum(mask[monthlyIdx,:])}, Selected: {sum(idxsSelected)}"
                )
            res_cov_window = Rdaily[
                (t + firstOOSDailyIdx - sizeCovarianceWindow + 1) : (
                    t + firstOOSDailyIdx + 1
            ...
            for i, factor in enumerate(factorList):
                if factor == 0:
                    residualsOOS[i : (i + 1), t : (t + 1), idxsSelected] = Rdaily[
                        (t + firstOOSDailyIdx) : (t + firstOOSDailyIdx + 1),
                        idxsSelected,
                    ]
