...
def run_famafrench():
    ff5 = pd.read_csv(
        "data/F-F_Research_Data_5_Factors_2x3_daily_CSV.zip", header=2, index_col=0
    )
    mom = pd.read_csv("data/F-F_Momentum_Factor_daily_CSV.zip", header=11, index_col=0)
    strev = pd.read_csv(
        "data/F-F_ST_Reversal_Factor_daily_CSV.zip", header=11, index_col=0
    )
    ltrev = pd.read_csv(
        "data/F-F_LT_Reversal_Factor_daily_CSV.zip", header=11, index_col=0
    )
    ...
    for cap in [0.01]:
        FamaFrench.OOSRollingWindowPermnos(
            cap=cap, save=True, sizeWindow=60, listFactors=[0, 1, 3, 5, 8]
        )
...
def init_argparse():
    ...
    parser.add_argument(
        "--model",
        "-m",
        help="the name of a factor model ('ipca', 'pca', or 'famafrench')",
        required=True,
    )
    return parser
