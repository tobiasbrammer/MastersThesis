...
def get_wrds(start_date="1997-12-31", end_date="2024-01-01"):
    ...
    fundq_vars = [
        ...
        "txdbq",  # Deferred taxes: txdb
        "itccy",  # Investment tax credit: itcb
        "saleq",  # Sales
        "cogsq",  # Cost of goods sold
        "xsgaq",  # Selling, general and administrative expenses
        "xintq",  # Interest expense
        "txditcq",  # Deferred income taxes
        "seqq",  # Shareholders' equity: seq
        "ceqq",  # Common equity
        "pstkq",  # Preferred stock - total: pstk
        "pstkrq",  # Preferred stock - redeemable: pstkrv / pstkl
        "niq",  # Net income
        "dpq",  # Dividends paid
    ...
    crsp_m = pd.merge(crsp_m, crsp_summe, how='inner', on=['jdate', 'permco'])
    del crsp_summe
    # Drop rename me_comp_x to me_comp
    crsp_m = crsp_m.drop(columns=['me_comp_x']).rename(columns={'me_comp_y': 'me_comp'})
    ...
    comp1 = pd.merge(ccm2, crsp_m[['permno', 'permco', 'date', 'jdate', 'siccd', 'me', 'me_comp']], how='inner',
